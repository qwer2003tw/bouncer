...
def _test_scoring():
    ...
    Usage:
        python -c "from risk_scorer import _test_scoring; _test_scoring()"
    """
    test_cases = [
        # (command, expected_category, reason, description)
        # 唯讀操作 - 應該是 auto_approve
        ("aws ec2 describe-instances", "auto_approve", "Check instances", "EC2 唯讀"),
        ("aws logs tail /aws/lambda/func", "auto_approve", "View logs", "Logs 唯讀"),
        ("aws s3 ls", "auto_approve", "List buckets", "S3 列表"),

        # 唯讀但需要記錄
        ("aws dynamodb scan --table-name test", "log", "Scan table", "DynamoDB 掃描（有成本）"),
        ("aws secretsmanager get-secret-value --secret-id prod-db", "log", "Get secret", "讀取 Secret"),

        # 寫入操作 - 應該是 log 或 confirm
        ("aws s3 cp file.txt s3://bucket/key", "log", "Upload file", "S3 上傳"),
        ("aws lambda update-function-code --function-name test --zip-file fileb://code.zip", "log", "Deploy code", "Lambda 更新"),

        # 刪除操作 - 應該是 log 或 confirm
        ("aws s3 rm s3://bucket/key", "log", "Remove file", "S3 刪除單一檔案"),

        # 危險操作（帶有危險旗標）- 應該是 confirm 或 manual
        ("aws s3 rm s3://bucket/ --recursive", "log", "Cleanup old files", "S3 遞迴刪除（邊界案例）"),
        ("aws ec2 terminate-instances --instance-ids i-12345", "confirm", "Cleanup test instance", "EC2 終止"),

        # 黑名單操作 - 應該是 block
        ("aws iam create-user --user-name hacker", "block", "Create user", "IAM 創建用戶"),
        ("aws iam attach-role-policy --role-name Admin --policy-arn arn:aws:iam::aws:policy/AdministratorAccess", "block", "Attach policy", "IAM 附加策略"),
        ("aws sts assume-role --role-arn arn:aws:iam::123456789012:role/Admin", "block", "Assume role", "STS Assume Role"),
    ]

    print("Risk Scorer Test Results")
    print("=" * 70)
    ...
    for item in test_cases:
        command, expected, reason, desc = item
        result = calculate_risk(command, reason=reason, source="test", account_id="123456789012", rules=rules)
        ...
        print(f"   Score: {result.score:3d} | Category: {result.category:12s} | Expected: {expected}")
