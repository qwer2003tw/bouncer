...
# ============================================================================
# S8-002: Extract Error Lines from SFN History Tests
# ============================================================================


class TestExtractErrorLinesFromSfnHistory:
    """S8-002: extract_error_lines_from_sfn_history tests."""

    def _make_sfn_client_with_events(self, events: list) -> MagicMock:
        """Create a mock SFN client that returns the given events."""
    ...
    def test_extract_error_lines_from_sfn_history(self):
        """Extracts error lines from TaskFailed events in SFN history."""
        import sys
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src'))
        from deployer import extract_error_lines_from_sfn_history
        ...
        events = [
            {
                ...
                'taskFailedEventDetails': {
                    ...
                    'cause': 'CodeBuild build failed with status FAILED',
                }
            }
        ]
        mock_sfn = self._make_sfn_client_with_events(events)
        lines = extract_error_lines_from_sfn_history('arn:aws:states::exec', mock_sfn)

        assert len(lines) == 1
        assert 'CodeBuild' in lines[0] or 'TaskFailed' in lines[0]

        sys.path.pop(0)

    def test_extract_from_execution_failed_event(self):
        """Extracts error lines from ExecutionFailed events."""
        import sys
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src'))
        from deployer import extract_error_lines_from_sfn_history
        ...
        assert len(lines) >= 1
        assert any('error' in line.lower() or 'Runtime' in line for line in lines)

        sys.path.pop(0)

    def test_error_lines_deduplication(self):
        """Duplicate error messages are deduplicated."""
        import sys
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src'))
        from deployer import extract_error_lines_from_sfn_history
        ...
        mock_sfn = self._make_sfn_client_with_events(dup_events)
        lines = extract_error_lines_from_sfn_history('arn:aws:states::exec', mock_sfn)

        # Should be deduplicated — only 1 unique line
        assert len(lines) == 1

        sys.path.pop(0)

    def test_error_lines_capped_at_5(self):
        """Returns at most 5 error lines."""
        import sys
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src'))
        from deployer import extract_error_lines_from_sfn_history
        ...
        events = [
            {
                ...
                'taskFailedEventDetails': {
                    'error': f'Error{i}',
                    'cause': f'Distinct error cause number {i} — unique message',
                }
            }
            for i in range(10)
        ]
    ...
    def test_returns_empty_on_no_failure_events(self):
        """Returns empty list when no failure events in history."""
        import sys
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src'))
        from deployer import extract_error_lines_from_sfn_history
        ...
        events = [
            ...
            {'type': 'TaskSucceeded', 'taskSucceededEventDetails': {'output': '{}'}},
        ]
        mock_sfn = self._make_sfn_client_with_events(events)
        lines = extract_error_lines_from_sfn_history('arn:aws:states::exec', mock_sfn)
    ...
    def test_error_lines_stored_in_ddb(self, mock_dynamodb):
        """error_lines are stored in DDB when deploy status transitions to FAILED."""
        import sys
        if 'deployer' in sys.modules:
            del sys.modules['deployer']
        if 'src.deployer' in sys.modules:
        ...
        deployer.history_table = mock_dynamodb.Table('bouncer-deploy-history')
        deployer.locks_table = mock_dynamodb.Table('bouncer-deploy-locks')

        # Create a RUNNING deploy record with execution_arn
        deploy_id = 'deploy-error-lines-test'
        deployer.history_table.put_item(Item={
            'deploy_id': deploy_id,
            'project_id': 'test-project',
            'status': 'RUNNING',
            'execution_arn': 'arn:aws:states:us-east-1:123456789012:execution:test:deploy-err',
            'started_at': int(time.time()) - 60,
        })

        # Mock SFN: execution is FAILED, history has an error event
        mock_sfn = MagicMock()
                ...
                'events': [
                    {
                        ...
                        'taskFailedEventDetails': {
                            'error': 'States.TaskFailed',
                            'cause': 'sam deploy failed: resource already exists',
                        }
                    }
                ]
            }
        ...
        deployer.sfn_client = mock_sfn

        # Call get_deploy_status → should detect FAILED and store error_lines
        record = deployer.get_deploy_status(deploy_id)

        assert record['status'] == 'FAILED'
        # error_lines should be stored in both record and DDB
        assert 'error_lines' in record
        assert len(record['error_lines']) >= 1

        # Verify DDB was updated
        ddb_item = deployer.history_table.get_item(Key={'deploy_id': deploy_id})['Item']
        assert 'error_lines' in ddb_item
        assert len(ddb_item['error_lines']) >= 1
