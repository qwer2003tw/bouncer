...
class TestExecuteCommandSubprocessRegression:
    """Regression tests for subprocess-based AWS CLI execution.

    Background: Sprint 65 refactored execute_command to use subprocess instead
    of awscli in-process execution. These tests ensure the new implementation
    handles success/failure cases correctly.
    """

    def test_subprocess_zero_with_output_returns_output(self, app_module):
        """subprocess exit 0 with output → return output, not crash Lambda."""

        with patch('commands._run_aws_subprocess', return_value=(0, 'command output here\n', '')):
    ...
    def test_subprocess_zero_no_output_returns_warning(self, app_module):
        """subprocess exit 0 no output → return warning message."""

        with patch('commands._run_aws_subprocess', return_value=(0, '', '')):
            result = app_module.execute_command('aws s3 ls')
    ...
    def test_subprocess_nonzero_returns_exit_code(self, app_module):
        """subprocess exit 1 → return (exit code: 1), not crash Lambda."""

        with patch('commands._run_aws_subprocess', return_value=(1, '', '')):
            result = app_module.execute_command('aws s3 ls')

        # Should return exit code, not crash
        assert '(exit code: 1)' in result

    def test_subprocess_nonzero_with_stderr_returns_error(self, app_module):
        """subprocess exit 1 with stderr → return stderr + exit code."""

        with patch('commands._run_aws_subprocess',
                   return_value=(1, '', 'An error occurred (InvalidBucketName)\n')):
