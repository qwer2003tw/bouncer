...
# ---------------------------------------------------------------------------
# Tests — expires_in minimum validation (from Approach B)
# ---------------------------------------------------------------------------
...
def test_expires_in_below_minimum_returns_error(mocked_aws):
    tbl, mcp_presigned = mocked_aws
    result = mcp_presigned.mcp_tool_request_presigned(
        "req-20", _valid_args(expires_in=30)
    )
    body = _body(result)
    assert body["status"] == "error"
...
# ---------------------------------------------------------------------------
# Tests — rate limit (from Approach B)
# ---------------------------------------------------------------------------
...
def test_rate_limit_exceeded_returns_error(mocked_aws):
    ...
    with patch("mcp_presigned.check_rate_limit", side_effect=RateLimitExceeded("Rate limit exceeded")):
        result = mcp_presigned.mcp_tool_request_presigned("req-22", _valid_args())
...
# ---------------------------------------------------------------------------
# Tests — ClientError detailed message (from Approach B)
# ---------------------------------------------------------------------------
